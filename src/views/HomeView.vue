...
const createGame = async () => {
  ...
  try {
    const session = await gameStore.createSession({
      name: 'Моя игра',
      isPrivate: false,
      maxPlayers: 8
    })
    ...
    error.value = err.response?.data?.error || 'Не удалось создать игру'
  } finally {
    isLoading.value = false
  }
}

// Войти в существующую игру по ID
    ...
    <main class="flex-1 flex items-center justify-center relative z-10 px-6 py-12">
      <div class="max-w-6xl w-full">
        ...
        <div class="text-center mb-16">
          <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-800/50 border border-white/10 mb-6">
            <Sparkles class="w-4 h-4 text-violet-400" />
            <span class="text-sm text-slate-300">Онлайн игровая платформа</span>
          </div>
          
          <h1 class="text-6xl md:text-7xl font-bold mb-6">
            <span class="bg-gradient-to-r from-violet-400 via-cyan-400 to-violet-400 bg-clip-text text-transparent bg-[length:200%_auto] animate-gradient">
              OGB
            </span>
          </h1>
        ...
        <!-- Quick Stats (для авторизованных) -->
        <div v-if="isAuthenticated" class="mt-16 grid grid-cols-3 gap-4 max-w-2xl mx-auto">
          <div class="p-4 bg-slate-900/50 rounded-xl text-center">
            <p class="text-2xl font-bold text-white">{{ gameStore.sessionId ? '1' : '0' }}</p>
            <p class="text-xs text-slate-400">Активных игр</p>
          </div>
          <div class="p-4 bg-slate-900/50 rounded-xl text-center">
            <p class="text-2xl font-bold text-white">{{ userStore.friends?.length || 0 }}</p>
            <p class="text-xs text-slate-400">Друзей</p>
          </div>
          <div class="p-4 bg-slate-900/50 rounded-xl text-center">
