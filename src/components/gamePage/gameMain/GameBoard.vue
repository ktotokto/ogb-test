...
import { ZoomIn, ZoomOut, Maximize, Move, GripHorizontal } from 'lucide-vue-next'
...
const handleMouseMove = (event) => {
  if (boardRef.value) {
    ...
    mousePosition.value = {
      x: 50000 - Math.round((event.clientX - rect.left - panOffset.value.x) / zoom.value),
      y: 50000 -Math.round((event.clientY - rect.top - panOffset.value.y) / zoom.value)
    }
  }
}
...
<template>
  <div class="w-full h-full relative bg-slate-950 overflow-hidden">
    ...
    <div 
      ...
      class="absolute inset-0 board-pan-area cursor-default"
    ...
    >
      ...
      >
        <GameObject
          ...
          :key="obj.id"
          :object="obj"
          :is-selected="selectedObjects.has(obj.id)"
          :is-draggable="true"
          :is-resizable="obj.resizable !== false"
          :zoom="zoom"
          :grid-size="gridSize"
          :snap-to-grid="false"
          @select="handleObjectSelect"
          @move="handleObjectMove"
          @delete="handleObjectDelete"
          @duplicate="handleObjectDuplicate"
          @rotate="handleObjectRotate"
        />
      </div>
    </div>
    ...
    <div class="absolute bottom-6 right-6 px-4 py-2 rounded-xl glass-strong border border-white/10 text-sm font-mono">
    ...
    <div class="absolute bottom-6 left-6 px-4 py-3 rounded-xl glass-strong border border-white/10 text-xs space-y-1">
