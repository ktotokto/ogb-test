...
const categories = [
  ...
  { id: 'boards', label: 'Поля', icon: Layers },
...
const objectTypes = [
  ...
  { 
    ...
    icon: Coins,
    category: 'tokens',
    defaults: { width: 50, height: 50, count: 1, label: 'Ресурс', color: '#f59e0b' },
    editorFields: [
      { key: 'label', type: 'text', label: 'Название' },
  ...
  { 
    ...
    icon: Square,
    category: 'tokens',
    defaults: { width: 80, height: 80, characterImage: null, isEnemy: false, label: 'Герой', health: 100 },
    editorFields: [
  ...
  { 
    ...
    icon: Box,
    category: 'custom',
    defaults: { width: 120, height: 120, url: null, label: 'Объект', rotation: 0 },
    editorFields: [
      { key: 'label', type: 'text', label: 'Название' },
  ...
  { 
    ...
    icon: LayoutGrid,
    category: 'boards',
    defaults: { width: 200, height: 200, tileImage: null, tileType: 'default', connections: [], label: 'Тайл' },
    editorFields: [
      { key: 'label', type: 'text', label: 'Название' },
      { key: 'tileImage', type: 'image', label: 'Текстура' },
      { key: 'tileType', type: 'select', label: 'Тип', options: ['default', 'nature', 'water', 'mountain', 'road'] },
      { key: 'connections', type: 'multiselect', label: 'Соединения', options: ['north', 'south', 'east', 'west'] }
    ]
  }
]
...
const filteredTypes = computed(() => {
  ...
  if (selectedCategory.value !== 'all') {
    types = types.filter(t => t.category === selectedCategory.value)
  }
  if (searchQuery.value) {
    types = types.filter(t => t.label.toLowerCase().includes(searchQuery.value.toLowerCase()))
  }
  return types
})

// 📋 Список объектов на поле
const objects = computed(() => {
  let filtered = gameStore.objects || []
  if (searchQuery.value) {
    filtered = filtered.filter(obj => obj.label?.toLowerCase().includes(searchQuery.value.toLowerCase()))
  }
  if (selectedCategory.value !== 'all') {
  ...
  <div class="space-y-4 p-3">
    ...
    <div class="space-y-3">
      ...
      <div class="flex gap-1.5 overflow-x-auto pb-1 scrollbar-thin">
        <button v-for="cat in categories" :key="cat.id" @click="selectedCategory = cat.id"
          :class="['px-3.5 py-2 rounded-xl text-xs font-medium flex items-center gap-2 whitespace-nowrap transition-all duration-200',
    ...
    <div class="grid grid-cols-3 gap-2.5">
      <button v-for="type in filteredTypes" :key="type.id" @click="openEditor(type)"
        ...
        :title="type.label">
        <div class="relative">
          <component :is="type.icon" class="w-5 h-5 text-violet-400 group-hover:text-violet-300 transition-colors duration-200"/>
          <div class="absolute inset-0 blur-lg bg-violet-500/20 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-200"></div>
        </div>
        <span class="text-[11px] font-medium text-slate-300 group-hover:text-white text-center leading-tight transition-colors">{{ type.label }}</span>
        <Settings class="w-3.5 h-3.5 text-slate-500 opacity-0 group-hover:opacity-100 transition-all duration-200 group-hover:text-violet-400"/>
      </button>
    </div>

    <!-- ⚡ Быстрые пресеты -->
    <div v-if="selectedCategory === 'tokens'" class="flex gap-1.5 flex-wrap pt-1">
      <button @click="quickCreate('dice', { diceSides: 6, label: 'd6' })" 
        class="px-2.5 py-1.5 bg-slate-700/50 hover:bg-slate-600/70 rounded-lg text-[10px] font-medium text-slate-300 hover:text-white transition-all border border-white/5 hover:border-cyan-500/30">d6</button>
      <button @click="quickCreate('dice', { diceSides: 20, label: 'd20' })" 
        class="px-2.5 py-1.5 bg-slate-700/50 hover:bg-slate-600/70 rounded-lg text-[10px] font-medium text-slate-300 hover:text-white transition-all border border-white/5 hover:border-cyan-500/30">d20</button>
      <button @click="quickCreate('counter', { label: 'Золото', count: 0 })" 
        class="px-2.5 py-1.5 bg-slate-700/50 hover:bg-slate-600/70 rounded-lg text-[10px] font-medium text-amber-300 hover:text-amber-200 transition-all border border-white/5 hover:border-amber-500/30">💰</button>
      <button @click="quickCreate('character', { label: 'Герой' })" 
        class="px-2.5 py-1.5 bg-slate-700/50 hover:bg-slate-600/70 rounded-lg text-[10px] font-medium text-emerald-300 hover:text-emerald-200 transition-all border border-white/5 hover:border-emerald-500/30">👤</button>
    </div>
      ...
      <div class="max-h-44 overflow-y-auto space-y-2 pr-1 scrollbar-thin">
        <div v-for="obj in objects" :key="obj.id"
          ...
          <div class="min-w-0 cursor-pointer flex-1" @click="openEditor(objectTypes.find(t => t.id === obj.type), obj)">
            <h4 class="text-sm font-medium text-white truncate group-hover:text-violet-300 transition-colors">{{ obj.label }}</h4>
    ...
    <Teleport to="body">
      <div v-if="showEditor && editorObject" 
        ...
        <div class="bg-slate-900 border border-white/10 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-scale-in">
          <!-- Header -->
          <div class="p-4 border-b border-white/10 flex justify-between items-center bg-gradient-to-r from-slate-800/50 to-transparent">
            ...
            <button @click="showEditor = false" class="p-2 text-slate-400 hover:text-white hover:bg-white/5 rounded-lg transition-all">
              <X class="w-5 h-5"/>
            </button>
          </div>
          ...
          <div class="flex border-b border-white/10 bg-slate-800/30">
            <button @click="editorTab = 'basic'" :class="['flex-1 py-3 text-sm font-medium transition-all duration-200', 
              editorTab === 'basic' ? 'text-violet-400 border-b-2 border-violet-400 bg-violet-500/5' : 'text-slate-400 hover:text-slate-300 hover:bg-white/5']">
              Основные
            </button>
            <button @click="editorTab = 'appearance'" :class="['flex-1 py-3 text-sm font-medium transition-all duration-200', 
              editorTab === 'appearance' ? 'text-violet-400 border-b-2 border-violet-400 bg-violet-500/5' : 'text-slate-400 hover:text-slate-300 hover:bg-white/5']">
              Внешний вид
            </button>
            <button v-if="editorObject.type === 'card' || editorObject.type === 'model2d'" @click="editorTab = 'images'" :class="['flex-1 py-3 text-sm font-medium transition-all duration-200', 
              editorTab === 'images' ? 'text-violet-400 border-b-2 border-violet-400 bg-violet-500/5' : 'text-slate-400 hover:text-slate-300 hover:bg-white/5']">
              Изображения
            </button>
          </div>
          ...
          <div class="p-4 max-h-[55vh] overflow-y-auto space-y-4 scrollbar-thin">
            ...
            <template v-if="editorTab === 'basic'">
              <div v-for="field in objectTypes.find(t => t.id === editorObject.type)?.editorFields?.filter(f => f.type !== 'image' && f.type !== 'color')" 
                :key="field.key" class="space-y-2">
                <label class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">{{ field.label }}</label>
                ...
                <label v-else-if="field.type === 'checkbox'" class="flex items-center gap-3 cursor-pointer group">
                  <input v-model="editorObject[field.key]" type="checkbox" class="w-4.5 h-4.5 rounded bg-slate-800 border-white/10 accent-violet-500 cursor-pointer"/>
            ...
            <template v-if="editorTab === 'appearance'">
              <div class="grid grid-cols-2 gap-4">
                <div class="space-y-2">
                  <label class="text-[11px] font-semibold text-slate-400 uppercase">Ширина</label>
                  <input v-model.number="editorObject.width" type="number" min="20" max="500"
                    class="w-full px-3.5 py-2.5 bg-slate-800 border border-white/10 rounded-xl text-sm text-white focus:outline-none focus:border-violet-500"/>
                </div>
                <div class="space-y-2">
                  <label class="text-[11px] font-semibold text-slate-400 uppercase">Высота</label>
                  <input v-model.number="editorObject.height" type="number" min="20" max="500"
                    class="w-full px-3.5 py-2.5 bg-slate-800 border border-white/10 rounded-xl text-sm text-white focus:outline-none focus:border-violet-500"/>
                </div>
              </div>
              
              <div class="space-y-2">
                <label class="text-[11px] font-semibold text-slate-400 uppercase">Поворот: {{ editorObject.rotation }}°</label>
                <input v-model.number="editorObject.rotation" type="range" min="0" max="360" step="15" class="w-full accent-violet-500 h-2 bg-slate-700 rounded-lg"/>
                <div class="flex gap-2">
                  <button @click="editorObject.rotation = (editorObject.rotation - 90 + 360) % 360" class="flex-1 py-2 bg-slate-800 hover:bg-slate-700 rounded-xl text-xs text-slate-300 hover:text-white transition-all">−90°</button>
                  <button @click="editorObject.rotation = (editorObject.rotation + 90) % 360" class="flex-1 py-2 bg-slate-800 hover:bg-slate-700 rounded-xl text-xs text-slate-300 hover:text-white transition-all">+90°</button>
                </div>
              </div>

              <!-- Preview for dice -->
              <div v-if="editorObject.type === 'dice'" class="p-3 bg-slate-800/50 rounded-xl border border-white/5">
                <label class="text-[11px] font-semibold text-slate-400 uppercase mb-2.5 block">Предпросмотр</label>
                <div class="text-5xl text-center text-cyan-400 drop-shadow-lg">{{ ['⚀','⚁','⚂','⚃','⚄','⚅'][editorObject.value-1] || '🎲' }}</div>
              </div>
              
              <div v-if="editorObject.type === 'counter'" class="space-y-2">
                <label class="text-[11px] font-semibold text-slate-400 uppercase">Цвет</label>
                <div class="flex items-center gap-3">
                  <input v-model="editorObject.color" type="color" class="w-12 h-12 rounded-xl cursor-pointer border-2 border-white/10 bg-slate-800"/>
            ...
            <template v-if="editorTab === 'images'">
              <div v-for="field in objectTypes.find(t => t.id === editorObject.type)?.editorFields?.filter(f => f.type === 'image')" 
                :key="field.key" class="space-y-2">
                <label class="text-[11px] font-semibold text-slate-400 uppercase">{{ field.label }}</label>
                
                <div class="border-2 border-dashed border-white/10 rounded-xl p-4 text-center hover:border-violet-500/50 transition-colors cursor-pointer bg-slate-800/30 hover:bg-slate-800/50"
                  @click="$refs[`imgInput_${field.key}`]?.click()">
                  <img v-if="getField(editorObject, field.key)" :src="getField(editorObject, field.key)" 
                  ...
                  <input :ref="`imgInput_${field.key}`" type="file" accept="image/*" class="hidden"
                    @change="handleImageUpload(field.key, $event)"/>
                </div>
                
                <button v-if="getField(editorObject, field.key)" @click="setField(editorObject, field.key, null)"
                  class="w-full py-2 text-xs text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded-lg transition-all">
                  Удалить изображение
                </button>
              </div>
            </template>
          ...
          <div class="p-4 border-t border-white/10 bg-slate-800/30 flex gap-3">
            <button @click="showEditor = false" class="flex-1 py-2.5 bg-slate-700 hover:bg-slate-600 text-white rounded-xl font-medium transition-all">Отмена</button>
            <button @click="saveFromEditor" class="flex-1 py-2.5 bg-violet-600 hover:bg-violet-500 text-white rounded-xl font-medium flex items-center justify-center gap-2 transition-all shadow-lg shadow-violet-500/25">
              <Save class="w-4 h-4"/>
              {{ editorObject.editing ? 'Сохранить' : 'Создать' }}
            </button>
          </div>
        </div>
      </div>
